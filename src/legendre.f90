...
function quad(nodeA,nodeB,dnA,dnB,xy,numNodes)
    integer::ii,jj,nodeA,nodeB,dnA,dnB,numNodes
...
!    double precision,dimension(2,2)::tranJ,invtranJ
    ...
    do ii = 1,numNodes
        P(1,ii) = quad_der_zeta(eta,ii)
        P(2,ii) = quad_der_eta(zeta,ii)
    end do
...
!    write(*,*)
    ...
    do ii = 1,2
        if(dn(ii).eq.0) then
            N(ii) = quad_bas(zeta,eta,node(ii))
        else
            N(ii) = invJ(dn(ii),1)*quad_der_eta(zeta,node(ii)) + &
                & invJ(dn(ii),2)*quad_der_zeta(eta,node(ii))
        end if
    end do
